...
// ======================== GARAGE DATA ========================
const BODY_DEFS = [
    { id:'speedster', name:'SPEEDSTER', price:0, color:0xE65100 },
    { id:'driftking', name:'DRIFT KING', price:1500, color:0x1565C0 },
    { id:'monster', name:'MONSTER', price:4000, color:0x4CAF50 },
    { id:'aero', name:'AERO', price:8000, color:0xFFEB3B }
];
const COLOR_DEFS = [
...
if (!garage.ownedBodies.includes('speedster')) garage.ownedBodies.push('speedster');
if (!BODY_DEFS.find(b => b.id === garage.selectedBody)) garage.selectedBody = 'speedster';
...
function buildGenericVoxelCar(color, type, isPlayer) {
    const g = new THREE.Group();
    // Voxel-style wheel
    const makeWheel = (x,z, r=0.4, thick=0.45) => {
        const wg = new THREE.Group();
        const tire = new THREE.Mesh(new THREE.BoxGeometry(thick,r*2,r*2), M(0x222222));
        const rim = new THREE.Mesh(new THREE.BoxGeometry(thick+0.05,r*1.2,r*1.2), M(0xcccccc));
        wg.add(tire); wg.add(rim);
    ...
    if (type === 'monster') { r=0.55; wlX=1.2; wrX=1.2; }

    g.add(makeWheel(-wlX, wlZ, r)); g.add(makeWheel(wlX, wlZ, r));
    g.add(makeWheel(-wrX, wrZ, r)); g.add(makeWheel(wrX, wrZ, r));

    const wMat = new THREE.MeshStandardMaterial({color:0x333333, roughness:0.2, metalness:0.8});

    if (type === 'speedster' || type === 'aero') {
        const bp = new THREE.Mesh(new THREE.BoxGeometry(2.0, 0.4, 4.4), M(color));
        bp.position.y = 0.5; g.add(bp);
        const cab = new THREE.Mesh(new THREE.BoxGeometry(1.6, 0.4, 2.0), M(color));
        cab.position.set(0, 0.9, -0.2); g.add(cab);
        const ws = new THREE.Mesh(new THREE.BoxGeometry(1.65, 0.35, 2.05), wMat);
        ws.position.set(0, 0.88, -0.2); g.add(ws);
        
        // Stripes for speedster
        if (type === 'speedster') {
            const stripe = new THREE.Mesh(new THREE.BoxGeometry(0.6, 0.42, 4.42), M(0xE2E8F0));
            stripe.position.set(0, 0.5, 0); g.add(stripe);
        }
        if (type === 'aero') {
            const spoilerGeo = new THREE.BoxGeometry(1.8,0.1,0.5);
            const spoiler = new THREE.Mesh(spoilerGeo, M(0x111111));
            spoiler.position.set(0, 0.9, -2.0); g.add(spoiler);
            const leg1 = new THREE.Mesh(new THREE.BoxGeometry(0.1,0.3,0.1), M(0x111111));
            leg1.position.set(-0.6, 0.7, -2.0); g.add(leg1);
            const leg2 = new THREE.Mesh(new THREE.BoxGeometry(0.1,0.3,0.1), M(0x111111));
            leg2.position.set(0.6, 0.7, -2.0); g.add(leg2);
        }
    } else if (type === 'driftking') {
        const bp = new THREE.Mesh(new THREE.BoxGeometry(2.0, 0.5, 4.6), M(color));
        bp.position.y = 0.5; g.add(bp);
        const cab = new THREE.Mesh(new THREE.BoxGeometry(1.8, 0.6, 2.2), M(color));
        cab.position.set(0, 1.0, 0); g.add(cab);
        const ws = new THREE.Mesh(new THREE.BoxGeometry(1.85, 0.5, 2.25), wMat);
        ws.position.set(0, 0.98, 0); g.add(ws);
        const skirt = new THREE.Mesh(new THREE.BoxGeometry(2.1, 0.15, 4.2), M(0x111111));
        skirt.position.y = 0.25; g.add(skirt);
    } else if (type === 'monster') {
        const bp = new THREE.Mesh(new THREE.BoxGeometry(2.2, 0.6, 4.8), M(color));
        bp.position.y = 0.7; g.add(bp);
        const cab = new THREE.Mesh(new THREE.BoxGeometry(2.0, 0.7, 1.8), M(color));
        cab.position.set(0, 1.25, 0.5); g.add(cab);
        const ws = new THREE.Mesh(new THREE.BoxGeometry(2.05, 0.6, 1.85), wMat);
        ws.position.set(0, 1.25, 0.5); g.add(ws);
        const bed = new THREE.Mesh(new THREE.BoxGeometry(1.8, 0.3, 1.8), M(0x222222));
        bed.position.set(0, 1.05, -1.2); g.add(bed);
    } else {
        const bp = new THREE.Mesh(new THREE.BoxGeometry(2.0, 0.5, 4.5), M(color));
        bp.position.y = 0.5; g.add(bp);
    }

    // Front/Rear Bumpers & Lights
    const fb = new THREE.Mesh(new THREE.BoxGeometry(2.02, 0.2, 0.2), M(0x222222));
    fb.position.set(0, 0.4, (type === 'monster'? 2.3 : 2.15)); g.add(fb);
    const rb = new THREE.Mesh(new THREE.BoxGeometry(2.02, 0.2, 0.2), M(0x222222));
    rb.position.set(0, 0.4, (type === 'monster'? -2.3 : -2.15)); g.add(rb);
    
    // Brightness difference for player vs NPC
    const hlMult = isPlayer ? 1.0 : 1.5;
    const hl1 = new THREE.Mesh(new THREE.BoxGeometry(0.3, 0.2, 0.21), ME(0xffffdd, 1*hlMult));
    hl1.position.set(-0.7, 0.55, (type === 'monster'? 2.3 : 2.15)); g.add(hl1);
    const hl2 = new THREE.Mesh(new THREE.BoxGeometry(0.3, 0.2, 0.21), ME(0xffffdd, 1*hlMult));
    hl2.position.set(0.7, 0.55, (type === 'monster'? 2.3 : 2.15)); g.add(hl2);
    
    // Assign generic naming for brake lights (required by updateBrakeLights logic)
    [[-0.7,0.55,(type === 'monster'? -2.3 : -2.15)],[0.7,0.55,(type === 'monster'? -2.3 : -2.15)]].forEach((p,i) => {
        const tl = new THREE.Mesh(new THREE.BoxGeometry(0.3, 0.2, 0.21), ME(0xff0000, 0.8));
        tl.position.set(p[0],p[1],p[2]);
        if (isPlayer) tl.name = 'brakeLight' + i;
        g.add(tl);
    });
...
const BODY_ICONS = {}; // Will replace by 3D renders
...
function spawnEnvObject(type) {
    ...
    switch(type) {
        case 'highway':
            ...
            else if(r<0.92){ mesh=makeHayBale(); cat='rock'; }
        ...
        case 'sunset':
            ...
            else if(r<0.55){ mesh=makeBirch(); cat='tree'; }
...
const state = {
    ...
    // Ambulance/Taxi mode
    missionTimer: 0, missionDist: 0, missionRound: 1,
};
...
function updateTimerMode(dt) {
    ...
    // Target distance scales with round
    const baseDist = state.mode === 'ambulance' ? 2000 : 1500;
    const targetDist = baseDist + (state.missionRound - 1) * 300;

    // Win: reached destination — start next round
    if (state.missionDist >= targetDist) {
        ...
        // Notify player
        sfx.victory();
        showBiome(`${t('delivered')} 🎯 ${t('score')}: +${bonus}`);

        // Next round: less time, reset distance
        const baseTime = state.mode === 'ambulance' ? 75 : 60;
        const timeReduction = Math.min((state.missionRound - 1) * 5, 25); // -5s per round, max -25s
        state.missionTimer = Math.max(baseTime - timeReduction, 30);
        state.missionDist = 0;

        // More traffic each round — decrease spawn interval
        state.oTimer = 0;

        // Update round display
        ui.timerIcon.textContent = (state.mode === 'ambulance' ? '🚑' : '🚕') + ' ' + state.missionRound;
        return;
    }

    // Lose: time ran out
    if (state.missionTimer <= 0) {
        gameLose(t('timeUp') + ` (${t('score')}: ${state.missionRound - 1} ${state.mode === 'ambulance' ? '🚑' : '🚕'})`);
    }
}
...
function loop() {
    ...
    if (state.phase==='playing') {
        ...
        // Traffic density increases with score + mission round
        const roundBonus = (state.mode === 'ambulance' || state.mode === 'taxi') ? (state.missionRound - 1) * 150 : 0;
        const spawnI = Math.max(500, 2000 - state.score*0.5 - roundBonus);
        ...
        for (let i=state.coinObjs.length-1; i>=0; i--) {
            ...
            if (ring) {
                ring.rotation.x = nowSec*2;
                ring.rotation.z = nowSec*1.5;
                const pulse = 0.9 + Math.sin(nowSec*6)*0.3;
        ...
        for (let i=state.boosts.length-1; i>=0; i--) {
            ...
            b.mesh.position.y = 0.8 + Math.sin(nowSec*4 + b.z*0.05) * 0.2;
            const r1 = b.mesh.getObjectByName('boostRing1');
            const r2 = b.mesh.getObjectByName('boostRing2');
            if (r1) { r1.rotation.x = nowSec*3; r1.rotation.z = nowSec*2; const s=0.8+Math.sin(nowSec*5)*0.4; r1.scale.set(s,s,s); }
            if (r2) { r2.rotation.x = nowSec*2.5+1; r2.rotation.y = nowSec*3; const s=0.7+Math.sin(nowSec*7)*0.3; r2.scale.set(s,s,s); }
...
window.garageTab = 'body';
window.setGarageTab = function(tab) {
    window.garageTab = tab;
...
let CAR_THUMBNAILS = {};
function preRenderCarThumbnails() {
    if (Object.keys(CAR_THUMBNAILS).length > 0) return;
    try {
        const tRend = new THREE.WebGLRenderer({ alpha: true, antialias: true });
        tRend.setSize(200, 150);
        const tCam = new THREE.OrthographicCamera(-3.5, 3.5, 2.5, -2.5, 0.1, 100);
        tCam.position.set(6, 6, -6);
        tCam.lookAt(0, 0.5, 0);
        ...
        BODY_DEFS.forEach(bd => {
            const builder = BODY_BUILDERS[bd.id] || buildGenericVoxelCar;
            const carGroup = builder(bd.color, true);
            tScene.add(carGroup);
            tRend.render(tScene, tCam);
            CAR_THUMBNAILS[bd.id] = tRend.domElement.toDataURL('image/png');
            tScene.remove(carGroup);
        });
        tRend.dispose();
    } catch(e) {
        console.error("Voxel thumbs err:", e);
    }
}

function renderGarage() {
    updateWalletDisplays();
    const carouselEl = document.getElementById('garage-carousel');
    if (!carouselEl) return;
    carouselEl.innerHTML = '';
    
    // Ensure thumbnails are generated
    preRenderCarThumbnails();
    
    if (window.garageTab === 'body') {
        BODY_DEFS.forEach(bd => {
            const owned = garage.ownedBodies.includes(bd.id);
            const selected = garage.selectedBody === bd.id;
            const card = document.createElement('div');
            
            // Add custom color class (g-speedster, g-driftking, etc) to apply border
            card.className = 'g-item g-' + bd.id + (selected ? ' selected' : owned ? ' owned' : (!owned ? ' locked' : ''));
            
            const thumbUrl = CAR_THUMBNAILS[bd.id] || '';
            const statusText = selected ? 'ACTIVE' : (owned ? 'OWNED' : 'LOCKED');
            const statusClass = selected ? 'selected' : (owned ? 'owned' : 'locked');
            
            card.innerHTML = 
                '<div class="g-item-title">' + bd.name + '</div>' +
                '<div class="g-item-status ' + statusClass + '">' + statusText + '</div>' +
                '<div style="height: 80px; width:100%; display:flex; justify-content:center; align-items:center;">' +
                    '<img style="width: 150px; filter: drop-shadow(0 10px 10px rgba(0,0,0,0.8));" src="' + thumbUrl + '" />' +
                '</div>' +
                '<div class="g-item-price-panel">' + 
                    '<div class="g-item-price">' + (owned ? '\u2713' : bd.price + ' COINS') + '</div>' +
                '</div>';
            ...
            card.onclick = () => {
                ...
                const updateModel = () => {
                    ...
                    playerCar = buildPlayerCar();
                    playerCar.position.set(0, 1000, 0);
                    scene.add(playerCar);
                };
    ...
    } else if (window.garageTab === 'color') {
        COLOR_DEFS.forEach((cd, i) => {
            const owned = garage.ownedColors.includes(i);
            const selected = garage.selectedColor === i;
            const card = document.createElement('div');
            card.className = 'g-item g-' + garage.selectedBody + (selected ? ' selected' : owned ? ' owned' : (!owned ? ' locked' : ''));
            const colorHex = '#' + cd.hex.toString(16).padStart(6, '0');
            
            const statusText = selected ? 'ACTIVE' : (owned ? 'OWNED' : 'LOCKED');
            const statusClass = selected ? 'selected' : (owned ? 'owned' : 'locked');
            
            card.innerHTML = 
                '<div class="g-item-title">' + cd.name + '</div>' +
                '<div class="g-item-status ' + statusClass + '">' + statusText + '</div>' +
                '<div style="height: 80px; width:100%; display:flex; justify-content:center; align-items:center;">' +
                    '<div class="g-color-preview' + (!owned ? ' locked' : '') + '" style="background: ' + colorHex + '; margin: 0; box-shadow: inset 0 0 15px rgba(0,0,0,0.8), 0 6px 0 rgba(0,0,0,1);"></div>' +
                '</div>' +
                '<div class="g-item-price-panel">' + 
                    '<div class="g-item-price">' + (owned ? '\u2713' : cd.price + ' COINS') + '</div>' +
                '</div>';
    ...
    } else if (window.garageTab === 'upgrades') {
        UPGRADE_DEFS.forEach(ud => {
            ...
            const card = document.createElement('div');
            card.className = 'g-item g-' + garage.selectedBody + (isMax ? ' selected' : '');
            
            let barsHtml = '';
            for (let i = 1; i <= maxLevel; i++) {
                barsHtml += '<div class="g-upgrade-seg' + (i <= level ? ' filled' : '') + '"></div>';
            }
            
            const statusText = isMax ? 'MAX LEVEL' : 'UPGRADE';
            const statusClass = isMax ? 'selected' : 'owned';

            card.innerHTML = 
                '<div class="g-item-title">' + ud.name + '</div>' +
                '<div class="g-item-status ' + statusClass + '">' + statusText + '</div>' +
                '<div style="height: 80px; width:100%; display:flex; flex-direction:column; justify-content:center; align-items:center;">' +
                    '<div style="font-size: 35px; margin-bottom:10px; filter: drop-shadow(0 3px 0 rgba(0,0,0,0.5));">' + ud.icon + '</div>' +
                    '<div class="g-upgrade-bars">' + barsHtml + '</div>' +
                '</div>' +
                '<div class="g-item-price-panel">' + 
                    '<div class="g-item-price">' + (isMax ? '\u2713' : cost + ' COINS') + '</div>' +
                '</div>';
...
function createGarageEnvironment() {
    if (garageEnvGroup) return;
    garageEnvGroup = new THREE.Group();
    garageEnvGroup.position.set(0, 1000, 0);

    // Premium Glossy Showroom Pedestal
    const platformGeo = new THREE.CylinderGeometry(7, 7.5, 0.5, 64);
    ...
    garageEnvGroup.add(platform);

    // Showroom Grid
    const gridHelper = new THREE.GridHelper(14, 14, 0x42A5F5, 0x90CAF9);
    gridHelper.position.set(0, 0.01, 0);
    
    // Create a circular mask for the grid so it doesn't bleed off the cylinder
    // By keeping it slightly smaller than the cylinder radius
    garageEnvGroup.add(gridHelper);

    // Neon accent ring
    const borderGeo = new THREE.CylinderGeometry(7.2, 7.7, 0.6, 64);
    const borderMat = new THREE.MeshBasicMaterial({ color: 0x29B6F6 });
    const border = new THREE.Mesh(borderGeo, borderMat);
    border.position.set(0, -0.35, 0);
    garageEnvGroup.add(border);

    // SHOWROOM LIGHTING
    const showLight = new THREE.DirectionalLight(0xffffff, 1.2);
    showLight.position.set(5, 10, 8);
    garageEnvGroup.add(showLight);
    
    const fillLight = new THREE.DirectionalLight(0xe3f2fd, 0.6);
    fillLight.position.set(-8, 5, -5);
    garageEnvGroup.add(fillLight);

    const ambient = new THREE.AmbientLight(0xffffff, 0.6);
    garageEnvGroup.add(ambient);

    scene.add(garageEnvGroup);
    garageEnvGroup.visible = false;
}

function showGarage() {
    renderGarage();
    document.getElementById('garage').style.display = 'flex';
    document.getElementById('garage-carousel').scrollLeft = 0;
    ...
    if (camera && playerCar) {
        if (!garageEnvGroup) createGarageEnvironment();
        garageEnvGroup.visible = true;
        
        playerCar.position.set(0, 1000, 0);
        
        // Position camera to look ABOVE the car so the car renders higher on screen
        camera.position.set(5.5, 1003.5, 6.5);
        camera.lookAt(0, 1001.5, 0); // Look at Y=1001.5 so car shifts up
...
function hideGarage() {
    ...
    if (camera && playerCar) {
        if (garageEnvGroup) garageEnvGroup.visible = false;
        
        // Return car and camera to ground
        playerCar.position.set(0, 0, 0);
        camera.position.set(0, 4, -8);
        camera.lookAt(0, 1, 10);
        
        // Restore environment background
        if (typeof state !== 'undefined' && BIOMES[state.biomeIdx]) {
            scene.background = new THREE.Color(BIOMES[state.biomeIdx].sky);
        } else {
...
window.selectedMenuMode = 'endless';
const menuModes = ['endless', 'chase', 'duel', 'fuel', 'cop', 'ambulance', 'taxi'];
menuModes.forEach(m => {
    const btn = document.getElementById('btn-'+m);
    if(btn) btn.onclick = () => {
        window.selectedMenuMode = m;
        menuModes.forEach(x => { const e = document.getElementById('btn-'+x); if(e) e.classList.remove('selected'); });
        btn.classList.add('selected');
    };
});
const globalPlayBtn = document.getElementById('btn-global-play');
if (globalPlayBtn) globalPlayBtn.onclick = () => startGame(window.selectedMenuMode);
...
function applyTranslations() {
    ...
    if (garageBtn) garageBtn.innerHTML = '<svg viewBox="0 0 24 24" width="24" height="24" stroke="currentColor" stroke-width="2" fill="none" stroke-linecap="round" stroke-linejoin="round" style="vertical-align: middle; margin-right: 6px;"><path d="M14.7 6.3a1 1 0 0 0 0 1.4l1.6 1.6a1 1 0 0 0 1.4 0l3.77-3.77a6 6 0 0 1-7.94 7.94l-6.91 6.91a2.12 2.12 0 0 1-3-3l6.91-6.91a6 6 0 0 1 7.94-7.94l-3.76 3.76z"></path></svg> ' + t('garage');
    const missTitle = document.querySelector('.missions-title');
    if (missTitle) missTitle.innerHTML = 'ЗАДАНИЯ ДНЯ';
...
// One-time bonus
if (!localStorage.getItem('rr3d_bonus_10k')) {
    garage.wallet += 10000;
    saveGarage();
    localStorage.setItem('rr3d_bonus_10k', '1');
}
updateWalletDisplays();
